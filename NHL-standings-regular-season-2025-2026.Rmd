...
NOTE: The R code has been omitted in this section to ensure maximum clarity of the tables. The codes will be presented later in section 10.
...
make_division_plot_with_wildcard_label <- function(df_div, division_name, x_scale, y_scale, colors = NULL) {
  ...
  ggplot(df_div, aes(x = gameDate, y = CumPoints, color = Code, group = Code)) +
    geom_line(linewidth = 0.5, alpha = 0.5) +
    geom_point(aes(fill = side), shape = 21, size = 1.5) +
    scale_color_discrete(labels = labels_vec) +
    ...
    theme(palette.colour.discrete = c("orange", "blue", "red", "cyan",
                                      "grey50", "grey80", "grey80", "grey80"))
}

div_order_names <- c("Atlantic", "Metropolitan", "Central", "Pacific")
plots_list <- lapply(div_order_names, function(d) {
  df_div_points <- filter(team_points_cum, Division == d)
  # Align to standings order for legend consistency
  div_levels <- division_tables[[d]]$Code
  df_div_points <- df_div_points %>% mutate(Code = factor(Code, levels = div_levels))
  make_division_plot_with_wildcard_label(df_div_points, d, global_x_scale, global_y_scale)
})
...
NOTE: The tables produced by these codes are presented in section 8.
