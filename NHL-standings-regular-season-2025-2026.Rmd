...
Saijo, Rento (2026). nhlscraper: Scraper for National Hockey League Data. R package version 0.4.1.9000, <https://rentosaijo.github.io/nhlscraper/>.
...
TeamIds <- games %>% 
  group_by(homeTeamId) %>% summarise() %>% rename(id = homeTeamId)

teams <- all_teams %>% filter(id %in% TeamIds$id)
...
games_enriched <- games %>%
  left_join(teams, by = c("homeTeamId" = "id")) %>%
  rename(
  ...
  left_join(teams, by = c("visitingTeamId" = "id")) %>%
  rename(
...
games_played <- games_enriched %>%
  select(id,            # NHL game id (might be needed later)
